...
from hr_prometheus.monitors import _path_with_fixed_parameters, _resolve_path, RequestMonitor
...
@pytest.mark.parametrize('wrong_fixed_routes_parameters', [
    (["resource_id"]),
    (23,),
    ({"route_name": {'some': 'dict'}}),
    ({"route_name": 16}),
])
def test_request_monitor_wrong_fixed_routes_parameters(mocker, wrong_fixed_routes_parameters):
    request_mock = mocker.Mock(method="GET", path="/path")
    request_mock.match_info.route.name = "route_name"

    with pytest.raises(ValueError):
        RequestMonitor(request_mock, fixed_routes_parameters=wrong_fixed_routes_parameters)
